...
describe('/api/custom-modules', () => {
  ...
  const exec = async (injectBool, realUser, payload) => {
    ...
    return request(server)
      .post('/api/automod-modules/massmentions-filter')
      .send({
        ...injector,
        ...payload
      });
  ...
  describe('POST /massmentions-filter', () => {
    ...
    it('should return 400 if limit property is not valid', async () => {
      ...
      const res2 = await exec(true, true, payload);
      expect(res2.status).toBe(400);
      expect(res2.text).toBe("Limit must be a number");

      // limit is less than 3
      payload.limit = 2;
      const res3 = await exec(true, true, payload);
      expect(res3.status).toBe(400);
      expect(res3.text).toBe("Limit must be at least 3");

      // limit is greater than 20
      payload.limit = 21;

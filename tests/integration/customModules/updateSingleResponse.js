...
describe('/api/custom-modules', () => {
  ...
  describe('PUT /update-single-response', () => {
    ...
    it('should return 400 if module id is not provided', async () => {
      ...
      expect(res.status).toBe(400);
      expect(res.text).toBe("Module ID is required");
    });
    ...
    it('should return 200 if single-response update is successful', async () => {
      const res = await exec(true, true, payload);

      // Should also test here that no new modules were added and that 
      // our module to update is showing updated values
      expect(res.status).toBe(200);      
    });
  });
});
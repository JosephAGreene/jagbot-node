...
describe('/api/automod-modules', () => {
  ...
  const exec = async (injectBool, realUser, payload) => {
    ...
    return request(server)
      .post('/api/automod-modules/word-filter')
      .send({
        ...injector,
        ...payload
      });
  ...
  describe('POST /word-filter', () => {
    ...
    it('should return 400 if enabled property is not valid', async () => {
    ...
    it('should return 400 if triggerWords property is not valid', async () => {
      // triggerWords property does not exist
      delete payload.triggerWords;
      const res1 = await exec(true, true, payload);
      expect(res1.status).toBe(400);
      expect(res1.text).toBe("Trigger words property is required");

      // triggerWords property is not an array
      ...
      expect(res4.text).toBe("At least 1 trigger word is required when enabled is true");

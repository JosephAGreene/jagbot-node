...
describe('/api/custom-modules', () => {
  ...
  describe('POST /update-optioned-response', () => {
    ...
    it('should return 200 if optioned-response update is successful', async () => {
      const res = await exec(true, true, payload);

      expect(res.status).toBe(200);
      expect(res.body.commandModules.length).toBe(2); // Ensure commandModules array did not grow    
    });
  });
});